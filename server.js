...
app.post("/register", async (req, res) => {
  try {
    ...
    }// TEMPORARY OWNER RECOVERY — REMOVE AFTER USE
app.all("/__recover_owner__", async (req, res) => {
  try {
    const username = (req.body?.username || req.query?.username || "").toString();
    const newPassword = (req.body?.newPassword || req.query?.newPassword || "").toString();
    const secret = (req.body?.secret || req.query?.secret || "").toString();

    if (!process.env.OWNER_RECOVERY_SECRET) {
      return res.status(500).send("OWNER_RECOVERY_SECRET is not set on the server env.");
    }
    ...
    const results = { pg: null, sqlite: null };
    ...
    if (process.env.DATABASE_URL) {
      ...
      // Try common hash column names
      const pgCols = ["passhash", "password_hash", "hash", "pw_hash", "password"];
      let updated = false;
      for (const col of pgCols) {
        try {
          const r = await pool.query(
            `UPDATE users SET ${col}=$1 WHERE lower(username)=lower($2) RETURNING id, username`,
            [hash, username]
          );
          if (r.rowCount > 0) {
            results.pg = { ok: true, col, row: r.rows[0] };
            updated = true;
            break;
          }
        } catch (e) {
          // ignore unknown column errors and keep trying
        }
      }
      if (!updated) results.pg = { ok: false, msg: "No PG row updated (user missing or unknown schema)." };
      await pool.end();
    } else {
      results.pg = { ok: false, msg: "DATABASE_URL not set (skipped PG)." };
    }

    // --- SQLite (auto-detect password + username columns on a users-like table)
    const sqlite3 = require("sqlite3").verbose();
    const path = require("path");
    const DB_FILE = process.env.DB_FILE || path.join(__dirname, "chat.db");
    ...
    const run = (sql, params=[]) => new Promise((resolve, reject)=>db.run(sql, params, function(e){ e?reject(e):resolve(this.changes); }));
    ...
    for (const t of tables.map(x => x.name)) {
      ...
      const ucol = USERNAME_COLS.find(c => names.includes(c));
      const pcol = PASS_COLS.find(c => names.includes(c));
      if (ucol && pcol) { found = { t, ucol, pcol }; break; }
    }

    if (!found) {
      results.sqlite = { ok: false, msg: "Could not find a table with username+password columns in SQLite." };
    } else {
      const changes = await run(
        `UPDATE ${found.t} SET ${found.pcol}=? WHERE lower(${found.ucol})=lower(?)`,
        [hash, username]
      );
      results.sqlite = { ok: changes > 0, table: found.t, userCol: found.ucol, passCol: found.pcol, changes };
    }
