...
function renderSurvivalArena() {
  ...
  const ZONE_POINTS = {
    "Forest": { x: 22, y: 28 },
    "Ruins": { x: 55, y: 28 },
    "High Ground": { x: 85, y: 30 },
    "Abandoned Camp": { x: 22, y: 78 },
    "Open Field": { x: 55, y: 75 },
    "Caves": { x: 85, y: 78 },
    "River": { x: 55, y: 46 },
    "Supply Drop": { x: 85, y: 62 },
  };
  ...
  if (fog === "hidden") {
    ...
    if (survivalArenaHint) survivalArenaHint.textContent = "Tap a token to see who it is.";
  }
  ...
  if (fog !== "hidden") {
    ...
    for (const p of ordered) {
      ...
      const isRealUser = Number(p.user_id || 0) > 0;
      const avatarUrl = isRealUser ? (p.avatar_url || "") : "";
      if (avatarUrl) {
        dot.classList.add('isAvatar');
